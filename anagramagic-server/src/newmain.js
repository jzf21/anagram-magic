...
const io = new Server(server, {
  cors: {
    origin: [
      "http://localhost:5173",
      "https://collaber.netlify.app",
      "https://collaber.onrender.com/",
      "https://collaber.onrender.com",
    ],
    methods: ["GET", "POST"],
  },
...
io.on("connection", (socket) => {
  ...
  socket.on("join_room", async (data) => {
    ...
    socket.join(roomID);
    socket.emit("room_joined",data.username, roomID);
    ...
    socket.to(data.room).emit("receive-message", messageData);
  });

  //   socket.on("send_message", (data) => {
  //     socket.to(data.room).emit("receive_message", data);
  //   });
  socket.on("start-round", (randomString,roomID) => {
    socket.to(roomID).emit("start-round", randomString);
    socket.to(roomID).emit("set-word",randomString);
  });
  socket.on("set-word", (word,roomID) => {
    socket.to(roomID).emit("set-word", word);
  });
  socket.on("set-score", (score, username,roomID) => {
